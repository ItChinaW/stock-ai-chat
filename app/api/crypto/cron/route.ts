...
import { selectStrategy } from "@/lib/ai-strategy-selector";
...
export async function GET(req: NextRequest) {
  ...
  const results: { id: number; symbol: string; action: string; strategy?: string; reason?: string; error?: string }[] = [];
  ...
  for (const bot of bots) {
    try {
      ...
      if (bot.aiMode) {
        const selected = selectStrategy(candles, bot.aiMode as "short" | "medium" | "long");
        strategyCode = selected.code;
        params = selected.params;
        aiReason = selected.reason;
        // 更新 bot 当前使用的策略（记录 AI 选择）
      ...
      results.push({ id: bot.id, symbol: bot.symbol, action, strategy: strategyCode, reason: aiReason });
    } catch (err) {
